// 14_WeatherMCPServer - 天气查询 MCP 服务器（HTTP 版本）
//
// 对应 Python: weather-mcp-server/server.py
// 天气查询 MCP 服务器，使用 HTTP 传输
...
func main() {
	...
	fmt.Println("🌤️  Starting Weather MCP Server...")
	...
	http.HandleFunc("/mcp", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"status":"ok","server":"weather-server"}`)
	})
